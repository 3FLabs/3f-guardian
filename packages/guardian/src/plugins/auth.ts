...
import { rawBodyPlugin } from "./raw-body.js";
    ...
    new Elysia({ name: `guardian:auth:${requiredScope}` })
      ...
      .derive({ as: "scoped" }, async ({ request, rawBody, set, logger }) => {
        ...
        if (!authz || !authz.toLowerCase().startsWith("bearer ")) {
          logger.warn({ scope: requiredScope }, "auth: missing or malformed Authorization");
        ...
        if (token.length === 0) {
          logger.warn({ scope: requiredScope }, "auth: empty bearer token");
          throw new UnauthenticatedError({
        ...
        return { tokenInfo, logger: requestLogger };
      })
      .as("scoped");
}

function requireSecret(token: TokenInfo, logger: import("../lib/logger.js").Logger): string {
  if (!token.hmacSecret) {
    ...
    logger.error("auth: requiresHmac true but hmacSecret missing in TokenInfo");
    throw new UnauthenticatedError({
...
function verifyHmac(args: {
  ...
  if (!/^\d+$/.test(tsHeader)) {
    logger.warn({ tsHeader }, "auth: HMAC timestamp not ASCII decimal");
    throw new UnauthenticatedError({
  ...
  if (skew > HMAC_TIMESTAMP_TOLERANCE_SECONDS) {
    logger.warn({ skew }, "auth: HMAC timestamp outside tolerance window");
    throw new UnauthenticatedError({
