 ...
 *
 * Keep every deadline below the HTTP server's idle timeout (Bun.serve
 * defaults to 10 s), otherwise the socket is reset before the 503
 * envelope can be written.
 */
export type GuardianTimeouts = {
  /** Deadline for `authenticate(token)`. Default 5 000 ms. */
  readonly authenticateMs?: number;
  /** Deadline for `accountRateLimit(tokenInfo)`. Default 5 000 ms. */
  readonly rateLimitMs?: number;
  /** Deadline for `liveness()` (GET /health). Default 5 000 ms. */
  readonly livenessMs?: number;
  /** Deadline for a whole `sign*` validate-and-sign call. Default 8 000 ms. */
  readonly signMs?: number;
};

/** Conservative defaults for {@link GuardianTimeouts}. */
export const DEFAULT_GUARDIAN_TIMEOUTS = {
  authenticateMs: 5_000,
  rateLimitMs: 5_000,
  livenessMs: 5_000,
  signMs: 8_000,
...
export function resolveGuardianTimeouts(overrides?: GuardianTimeouts): ResolvedGuardianTimeouts {
  return {
    authenticateMs: overrides?.authenticateMs ?? DEFAULT_GUARDIAN_TIMEOUTS.authenticateMs,
    rateLimitMs: overrides?.rateLimitMs ?? DEFAULT_GUARDIAN_TIMEOUTS.rateLimitMs,
    livenessMs: overrides?.livenessMs ?? DEFAULT_GUARDIAN_TIMEOUTS.livenessMs,
    signMs: overrides?.signMs ?? DEFAULT_GUARDIAN_TIMEOUTS.signMs,
  };
}
